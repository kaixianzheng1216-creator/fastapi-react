...
from app.modules.users import service as crud
...
def test_create_user_new_email(
    ...
    with (
        patch("app.modules.users.router.send_email", return_value=None),
...
def test_delete_user_super_user(
    ...
    user = crud.create_user(session=db, user_create=user_in)
    user_id = user.id
    item = items_service.create_item_for_owner(
        session=db,
        item_create=ItemCreate(title=random_lower_string()),
        owner_id=user_id,
    )
    item_id = item.id
    r = client.delete(
