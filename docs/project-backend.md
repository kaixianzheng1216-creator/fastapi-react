...
- `read_only`：仅开放已启用知识库列表与检索，以及三个平台业务数据查询工具。
- `read_write`：包含全部业务查询能力，额外允许维护该项目全部知识库及文档。
...
本次审核已通过前端类型检查、生产构建、后端 Ruff/Mypy、7 项隔离数据库集成测试、13 项组件检查和 19 项浏览器回归。测试覆盖项目隔离、权限、密钥撤销、表单及筛选交互；对象存储上传使用模拟地址，实际文件解析和向量服务未做完整端到端验证。

按本次交付要求，临时测试脚本、报告及新增的集成测试文件已移除。以下检查仍可执行：
