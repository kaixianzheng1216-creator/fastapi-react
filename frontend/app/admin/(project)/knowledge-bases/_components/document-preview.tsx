...
import { DownloadIcon, FileTextIcon, LayersIcon } from "lucide-react";
...
export function KnowledgeDocumentPreview({
  ...
  const documentPath = `${projectHref(project.id)}/${knowledgeBaseId}/documents/${documentId}`;
  ...
  const documentQuery = useQuery({
    ...
    queryKey: ["knowledge-document", knowledgeBaseId, documentId],
    queryFn: async ({ signal }) => {
      ...
      if (data.knowledge_base_id !== knowledgeBaseId)
        throw new Error("文档不属于当前知识库");
      return data;
    },
  });
  const documentState = getQueryViewState(documentQuery);
  ...
  const previewState = getQueryViewState(previewQuery);
  ...
  if (
    ...
    redirect(
      `${projectHref(knowledgeBaseQuery.data.project_id)}/${knowledgeBaseId}/documents/${documentId}`,
    );
  }
  ...
  return (
    <>
      <AppHeader
        ...
        breadcrumbs={[
          { label: "知识库", href: projectHref(project.id) },
          {
            label: knowledgeBaseQuery.data?.name ?? "知识库详情",
            href: directoryHref,
          },
        ]}
        actions={
...
function DocumentChunksView({
  ...
  const chunksState = getQueryViewState(
    chunksQuery,
  ...
  return (
    <CollectionContent
      ...
      inert={chunksState === "ready" && chunksQuery.isPlaceholderData}
